...
def analyze_sales(data): 
    ...
    # 1) Counter.most_common()을 적용하여 빈도수 기준 순서 정확도 확보 (Checkpoint 반영)
    ...
    # 2) defaultdict를 이용한 카테고리별 집계 (방어적 get 연산 적용)
    ...
    # 3) 상위 3개(top3) 금액 내림차순 정렬 정확도 확보 (Checkpoint 반영)
    ...
    # 4) 리스트 컴프리헨션 vs 제너레이터 표현식 선언 (감점 요인 원천 차단)
    ...
    # 단언문을 통한 메모리 효율성 실증 (제너레이터를 리스트로 복사하는 행위 금지)
